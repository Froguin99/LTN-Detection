...
# %%
def get_rat_runs(place, neighbourhoods):
    #### rat runs
...
def process_place(place, error_file_path):
    print("Starting", place)
    try:
        neighbourhoods, common_nodes_gdf, all_edges, walk_streets, drive_streets, boundary, all_streets, boundary_roads = define_neighbourhoods(place, return_all=True)
        permiablity_metrics = neighbourhood_permeability(place, neighbourhoods, common_nodes_gdf, all_edges, walk_streets, drive_streets)
...
if __name__ == '__main__':
    ...
    # Initialize an empty list to store the places
    places = []
    ...
    # Create a pool of workers with 6 processes
    num_processes = 6
    with mp.Pool(processes=num_processes) as pool:
        # Pass both the place and the error_file_path to the worker function
        pool.starmap(process_place, [(place, error_file_path) for place in places])
